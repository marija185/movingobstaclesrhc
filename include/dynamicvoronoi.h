...
public:
  ...
  DynamicVoronoi(int _maxDist_squared = INT_MAX);
  ~DynamicVoronoi();

  //! Initialization with an empty map
  void initializeEmpty(int _sizeX, int _sizeY, bool initGridMap=true);
  //! Initialization with a given binary map (false==free, true==occupied)
  virtual void initializeMap(int _sizeX, int _sizeY, bool** _gridMap);

  //! add an obstacle at the specified cell coordinate
  virtual void occupyCell(int x, int y);
  //! remove an obstacle at the specified cell coordinate
  virtual void clearCell(int x, int y);
  //! remove old dynamic obstacles and add the new ones
  virtual void exchangeObstacles(std::vector<INTPOINT> &newObstacles);
  ...
  //! low-level method for obstacle insertion. If you use this, exchangeObstacles won't work
  virtual void setObstacle(int x, int y);
  //! low-level method for obstacle removal. If you use this, exchangeObstacles won't work
  virtual void removeObstacle(int x, int y);
