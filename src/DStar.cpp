...
#include "GridMap.h"
#include "rvl_psulmdemo/predict_trace.h"
...
bool    DStar::SearchPath()
{
		...
		for(int i=0; (i<NumElemPraznjenja); i++)
		{
			...
			{//a pitamo za svaki slucaj da nije doletio neki za punjenje, sto nece biti slucaj al eto
				if ((map[element.x][element.y].tag==CLOSED)) //&&(map[element.x][element.y].h_cost_int<OBSTACLE))//injega
				{
					...
					//if change is affecting optimal path (blacklist) reset part of the grid connected with the change

					std::vector<I_point>::iterator it;
					it = std::find (blacklist.begin(), blacklist.end(), element);
					if ((it != blacklist.end()) && (blacklist.size()>0) ){

						//reset black list elements
						blacklist.clear();


						std::vector<I_point> terminator_list;
						terminator_list.push_back(element);
						I_point change;

						while (terminator_list.size()!=0) {
							//reset

							change=terminator_list.front();
							terminator_list.erase(terminator_list.begin()); //erase element from the list
							map[change.x][change.y].h_cost_int=OBSTACLE;       //set value to inf
							...
							for ( int d = 0; d < 8; d++ ) //for all neighbors
...
#endif
								{
									...
									point.x=change.x+xofs[d];
									point.y=change.y+yofs[d];
									if ( IsValid( point.x, point.y )!=0 ){
										if (map[point.x][point.y]._next.x==change.x && map[point.x][point.y]._next.y==change.y)


											terminator_list.push_back(point);
...
#endif
					{
						...
						if ( IsValid( point.x, point.y )!=0 )//moraju imati i prepreku u sebi
						{
							if ((map[point.x][point.y].tag==CLOSED))//&&(map[point.x][point.y].h_cost_int<OBSTACLE))//ciste susjede ubacujemo
							{
								...
								std::vector<I_point>::iterator it;
								it = find (blacklist.begin(), blacklist.end(), element);
								if ((it != blacklist.end()) && (blacklist.size()>0) ) {
									//reset black list elements
    ...
    //element.x=Start.x + 30;
        //element.y=Start.y - 5;
    map[negative_cell.x][negative_cell.y].traversal_cost=-1000;
    			                      					  ...
    			                      					  if ((it != blacklist.end()) && (blacklist.size()>0) ) {
    			                      						  //reset black list elements
    			                      												  ...
    			                      												  while (terminator_list.size()!=0) {
    			                      													  ...
    			                      													  change=terminator_list.front();
    			                      													  terminator_list.erase(terminator_list.begin()); //erase element from the list
    			                      													  map[change.x][change.y].h_cost_int=10000;       //set value to inf
   ...
   /* if ( (map[element.x][element.y].tag==CLOSED))// && (map[element.x][element.y].h_cost_int<OBSTACLE) ) //injega
	 ...
	 */
	//if (prviput==1){
	//find two localization points - first and the middle one
	//int temp_sl=loop_close_points.size();

	//compute values in the map



	if ((loop_close_points.size()>0)){

		//ako imam negativnu onda reset
		if (negative_cell.size()!=0) {
			map[negative_cell[0].x][negative_cell[0].y].traversal_cost=2;

			//if (negative_cell.size()==2)
			//map[negative_cell[1].x][negative_cell[1].y].traversal_cost=2;

			map[blacklist.back().x][blacklist.back().y].traversal_cost=2;
			if ( (map[negative_cell[0].x][negative_cell[0].y].tag==CLOSED))// && (map[element.x][element.y].h_cost_int<OBSTACLE) ) //injega
			{

				insertNode(negative_cell[0],map[negative_cell[0].x][negative_cell[0].y].h_cost_int);
				insertNode(blacklist.back(),map[blacklist.back().x][blacklist.back().y].h_cost_int);
				watchdog_counter++;
			}
			//	    		      negative_cell.x=-1;
			//	    		      negative_cell.y=-1;
		...
		if(negative_cell.size()==0) {
			...
			if (GM->check_point(GM->mapper_point_temp)==0)
			{
				printf("Negative point izvan the map");
			}
			else {
				printf("inside mape sick (%f,%f)\n",GM->mapper_point_temp.x, GM->mapper_point_temp.y);


				//negative_cell.clear();
				negative_cell.push_back(GM->cell_point_temp);
				negative_cell_forlog.push_back(GM->cell_point_temp);
				FILE *file_op7;
				file_op7 = fopen("/media/kruno/Data/TEST/SLAM/negativne.txt","a");
				//for(int i=0; i< negative_cell_forlog.size();i++)
				if (reset_log=true){
					fprintf(file_op7,"\n %d %d\n",negative_cell_forlog[negative_cell_forlog.size()-1].x, negative_cell_forlog[negative_cell_forlog.size()-1].y);
				reset_log=false;
				}
				else
				fprintf(file_op7,"%d %d\n",negative_cell_forlog[negative_cell_forlog.size()-1].x, negative_cell_forlog[negative_cell_forlog.size()-1].y);
				...
				/*if (loop_close_points.size()==2) {
			GM->mapper_point_temp.x=loop_close_points[1].x*1000;
			GM->mapper_point_temp.y=loop_close_points[1].y*1000;

			if (GM->check_point(GM->mapper_point_temp))
			{
				printf("Negative point inside the map");
			}
			else {
			...
			negative_cell_forlog.push_back(GM->cell_point_temp);
				...
				//rvl_psulmdemo::predict_trace srv;
				//srv.request.stateID=0;
				//ros::service::call("predict_trace", srv);
				int cost1=loop_close_points[0].cijena;
				//srv.request.stateID=temp_sl/2;
				//ros::service::call("predict_trace",srv);
				int cost2=1;
				if (loop_close_points.size()==2)
					cost2=loop_close_points[1].cijena;

				//skalirati trace matrice
				map[negative_cell[0].x][negative_cell[0].y].traversal_cost=cost1/(-500);
				//if (negative_cell.size()==2)
				//map[negative_cell[1].x][negative_cell[1].y].traversal_cost=int(cost2/cost1*(-50));

				//dodati na open listu
			...
			if ( (map[negative_cell[1].x][negative_cell[1].y].tag==CLOSED))// && (map[element.x][element.y].h_cost_int<OBSTACLE) ) //injega
			{


				insertNode(negative_cell[1],map[negative_cell[1].x][negative_cell[1].y].h_cost_int);
				watchdog_counter++;
			}
		}

		//map[negative_cell[1].x][negative_cell[1].y].traversal_cost=1;
	}
				 */
				//if (negative_cell.x!=-1)
				//  map[negative_cell.x][negative_cell.y].traversal_cost=-100;
				loop_close_points.clear();
	...
	std::vector<I_point>::iterator it;
	it = std::find (blacklist.begin(), blacklist.end(), Start);
	if ((it != blacklist.end()) && (blacklist.size()>0) ){
		map[negative_cell[0].x][negative_cell[0].y].traversal_cost=2;
		...
		rvl_psulmdemo::ExtCommand slam_ext_command;
		slam_ext_command.request.augment_state = true;
		slam_ext_command.request.stop_closing = true;
		slam_ext_command.request.match_state = true;
		ros::service::call("ext_command",slam_ext_command);

		blacklist.clear();
		negative_cell.clear();
	}

	if (negative_cell.size()>0) { //loop_close_points.size()>1){

		if ((Start - negative_cell[0]<10) && blacklist.size()>0 ){
			rvl_psulmdemo::ExtCommand slam_ext_command;
			slam_ext_command.request.augment_state = true;
			slam_ext_command.request.stop_closing = true;
			slam_ext_command.request.match_state = true;
		...
		/*	if (negative_cell.size()==2){
			if ((Start - negative_cell[1]==10) && blacklist.size()>0 ){
				rvl_psulmdemo::ExtCommand slam_ext_command;
				slam_ext_command.request.augment_state = true;
				slam_ext_command.request.stop_closing = true;
				slam_ext_command.request.match_state = true;
...
void DStar::reset()
{
	...
	FILE *file_op7;
	file_op7 = fopen("/media/kruno/Data/TEST/SLAM/negativne.txt","a");
	fprintf(file_op7,"reset %d\n",negative_cell_forlog.size());
	fclose(file_op7);

	negative_cell_forlog.clear();
