...
{
	FILE *pFile;
	pFile = fopen("/media/kruno/Data/TEST/SLAM/primo.txt","a");
	fprintf(pFile,"startao...\n");
	...
	for(int i=0;i<msg->poses.size();i++){
		...
		pom.cijena= (int)msg->poses[i].pose.position.z;
		fprintf(pFile,"%f %f %d\n",pom.x,pom.y,pom.cijena);
		DS->loop_close_points.push_back(pom);
	}
	fclose(pFile);
	//ROS_INFO("\n\n\n\n\n\nPRIMIO SAM OD ROBOTA NJEGOVU X: %f \n\n\n\n\n\n",robotX);
}
...
bool GenericLaserScanFilterNode::ReqRobotStop(rvl_psulmdemo::SendRobot::Request &req, rvl_psulmdemo::SendRobot::Response &res){
	...
	if (ciklus>10){
		...
		if (((ciklus % 10)==0)&&(voznja==false)){
			...
			if (slam_loop){
				...
				if (client_slam_extcomm.call(slam_ext_command)){
					if(slam_ext_command.response.sent){
						ROS_INFO("SLAM will augment and match!");
					}else{
